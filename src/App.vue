...
<template>
  ...
  <Lesson v-if="page===1" msg="List of all Lessons!!" :lessons="sortedLessons" @aId="addToCart($event)" :pageNo="this.page" :lspaces="lspaces" :sortby="sortby" :orderby="orderby" @filter="filterLessons($event)"/>
...
<script>
  ...
  methods:{
   ...
   emptyCart(obj){
          ...
          fetch('http://backend-env.eba-jqbtuzpq.eu-west-2.elasticbeanstalk.com/api/order',{
          method: "POST",  
          headers: {
        ...
        .then(res=>{
          ...
          fetch('http://backend-env.eba-jqbtuzpq.eu-west-2.elasticbeanstalk.com/api/lessons/update',{
          method: "PUT",  
          headers: {
            "Content-Type": "application/json",
          },
  ...
  mounted(){
    console.log("mounted")
    fetch('http://backend-env.eba-jqbtuzpq.eu-west-2.elasticbeanstalk.com/api/lessons',{
    method: "GET", 
    mode: "cors", 
    cache: "no-cache", 
